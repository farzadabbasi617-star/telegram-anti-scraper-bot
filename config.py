...
# 🔥 گرم کردن اکانت (warm-up)
# اکانتی که تازه اضافه شده یا سابقه ادد ندارد نباید روز اول ۱۰۰ نفر
# اضافه کند — مستقیم PEER_FLOOD می‌گیرد. سقف بر اساس مجموع اددهای
# تاریخی اکانت بالا می‌رود.
WARMUP_STAGES = (
    (0,   12),     # اکانت بدون سابقه → حداکثر ۱۲ ادد در این اجرا
    (20,  25),     # بعد از ۲۰ ادد موفق تاریخی → ۲۵
    (60,  45),     # بعد از ۶۰ → ۴۵
    (150, 70),     # بعد از ۱۵۰ → ۷۰
    (300, 100),    # باتجربه → سقف کامل
)

# فاصله شروع بین اکانت‌ها (ثانیه). اگر همه با هم شروع کنند، تلگرام
# الگوی هماهنگ می‌بیند.
STAGGER_START = {
    "ultra": (0, 5),
    "fast": (5, 25),
    "safe": (30, 120),
...
# ⚠️ درس گران (۱.۵.۸): با safe=(45,95) و ۸ اکانت موازی، یک ادد هر ~۹
# ثانیه به یک گروه می‌رسید — از دید تلگرام هجوم اسپم. پنج اکانت بعد از
# ۱ تا ۷ ادد PEER_FLOOD گرفتند. حالت safe باید واقعاً safe باشد.
DELAY_RANGES = {
    "ultra": (5, 10),
    "fast": (16, 38),
    "safe": (90, 180),
