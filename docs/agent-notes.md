...
## 🍯 هانی‌پات — با احتیاط دست بزن

**پیش‌فرض `HONEYPOT_MODE=off` است و دلیل دارد.**

تا نسخه ۱.۴.۰ ربات هر ۱۰ دقیقه پیامی با کد `hp_trap_123456` در گروه
می‌کاشت. توسعه‌دهنده فکر می‌کرد نامرئی است چون کاراکترهای zero-width
دورش گذاشته بود — ولی خودِ توکن داخل `<code>` بود و همه می‌دیدند.
**اعضای واقعی گروه آن را اسکم تصور کردند و گروه را ترک کردند.**

بدتر: هر کس آن کد را کپی می‌کرد یا می‌پرسید «این چیه؟» فوراً بن می‌شد،
چون شرط تشخیص فقط `"hp_trap_" in text` بود.

قوانین حالا:
- هیچ پیامی بدون تصمیم صریح مالک در گروه کاشته نمی‌شود
- تله (اگر روشن شود) هیچ متن قابل خواندنی ندارد و ۳ ثانیه بعد پاک می‌شود
- برخورد با تله **بن خودکار نمی‌آورد** — فقط امتیاز ریسک + گزارش به مالک
- فقط توکن‌هایی که در `self.active_traps` هستند اعتبار دارند
- `alert()` هرگز به `group_id` نمی‌فرستد، فقط به `admin_id`

`tests/test_defender_safety.py` نگهبان همه‌ی این‌هاست.

**قانون کلی: هر چیزی که در گروه کاربر منتشر می‌شود باید برای یک عضو
عادی قابل توضیح باشد.** اگر پیامی مرموز به نظر می‌رسد، اعتماد کاربران
را از بین می‌برد — هزینه‌اش خیلی بیشتر از فایده‌ی شکار چند اسکرپر است.
